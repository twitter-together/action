...
async function createCheckRun(
  ...
  // Check runs cannot and comments cannot be created if the pull request was created by a fork,
  // so we just log out the result and fallback to text-only rendering (no HTML for images, etc)
    ...
    const media = tweet.media
      .map(({ file, alt }) => {
        ...
        if (textOnly) {
          ...
          return `${alt}\n<img src="https://raw.githubusercontent.com/${payload.repository.owner.login}/${payload.repository.name}/${payload.after}${fileName}" height="200" />`;
        }
      })
      .join(textOnly ? "\n" : "\n\n");
    text = `${media}\n\n${text}`.trim();
