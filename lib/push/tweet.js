...
async function handleTweet(client, tweet, name) {
    ...
    if (tweet.reply) {
        ...
        const match = tweet.reply.match(/^https:\/\/twitter\.com\/[^/]+\/status\/(\d+)$/);
        if (match) {
    ...
    if (tweet.thread) tweetResult.thread = await handleTweet(client, { ...tweet.thread, reply: tweetResult.url }, name);
