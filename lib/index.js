...
async function main() {
  ...
  // do not make comments by default if "pull_request_target" is not used
  // but optionally allow enabling them on "pull_request" for local branches
  const comments =
    !!process.env.ENABLE_COMMENTS || trigger === "pull_request_target";
