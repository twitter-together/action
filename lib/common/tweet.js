...
async function handleTweet(client, self, tweet, name) {
  ...
  if (tweet.poll) {
    tweetData.poll = {
      ...
      options: tweet.poll.pollOptions,
    };
  }
  ...
  if (tweet.retweet) {
    // TODO: Should this throw if an invalid tweet is passed and there is no match?
    const match = tweet.retweet.match(TWEET_REGEX);
    if (match) tweetData.quote_tweet_id = match[0];
  }

  if (tweet.media) {
    tweetData.media = {
      media_ids: await Promise.all(tweet.media.map((media) => createMedia(client, media))),
    };
  }
...
async function createMedia(client, { file, alt }) {
  ...
  if (alt) await client.v1.createMediaMetadata(mediaId, { alt_text: { text: alt } });
  return mediaId;
}
...
function createRetweet(client, self, id) {
  return client.v2.retweet(self.id, id).then(async data => {
    ...
    const other = await client.v2.singleTweet(id, { 'user.fields': 'username' });
    if (other.errors) throw other.errors;
    const otherUser = other.includes.users.find(user => user.id === other.data.author_id);

    return {
      retweet: `https://twitter.com/${otherUser.username}/status/${id}`,
      url: '', // TODO: Twitter does not return the id of the retweet itself
    }
  });
}
