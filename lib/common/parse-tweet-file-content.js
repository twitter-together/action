...
function parseTweetFileContent(text, isThread = false) {
  ...
  if (frontMatterMatch) {
    ...
    if (isThread) {
      if (options.reply) throw new Error("Cannot set a tweet to reply to when in a thread");
    }
  }

  if (options.threadDelimiter) {
    const threadIdx = text.match(new RegExp(`(?:${EOL})+${options.threadDelimiter}(?:${EOL})+`));
    if (threadIdx) {
      // Each item can have front matter, as we only split one thread delimiter at a time
      options.thread = parseTweetFileContent(text.slice(threadIdx.index + threadIdx[0].length), true);
